import { CategoryChannel, ChannelType, ColorResolvable, EmbedBuilder, MessageFlags, OverwriteResolvable, StringSelectMenuInteraction } from "discord.js"
import { getTicketCustomIds } from "../utils/getTicketsCustomId"
import { getTicketByCustomId } from "../utils/getTicketByCustomId"
import { zeroFill } from "../utils/zeroFill"
import { serverName, defaultColor, defaultTitle, defaultDescription } from "../configs/generalConfig.json"
...
export async function buttonHandlerFunction(_: any, interaction: StringSelectMenuInteraction) {
  ...
  if (!createTicketsCustomIds.includes(interaction.values[0])) return interaction.reply({ content: "No ha sido posible crear el ticket. No existe el ID personalizado del botón.", flags: MessageFlags.Ephemeral })

  const ticketInfo = getTicketByCustomId(interaction.values[0])
  if (!ticketInfo) return interaction.reply({ content: "No ha sido posible crear el ticket. No existe el ID personalizado del botón. Informa de este error a un administrador", flags: MessageFlags.Ephemeral })
